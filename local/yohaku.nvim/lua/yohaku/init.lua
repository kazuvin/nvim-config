...
local defaults = {
  ...
  -- 書いている間のカーソル: "bar"（縦棒）/ "underline"（下線）/ "auto"。
  -- Ghostty 1.3 までは縦棒がマスの高さいっぱいに描かれる不具合があり、行間を広げると背が高くなる
  -- （1.4.0 で修正、開発版の tip には修正済み）。"auto" は修正済みの Ghostty なら縦棒、
  -- そうでなければ文字のすぐ下に付く下線にする。どちらも入力中だけ点滅する
  cursor = "auto",
